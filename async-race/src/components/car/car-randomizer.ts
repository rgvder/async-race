const carBrand: string[] = [
  ...
  'Dacia', 'Daewoo', 'Daihatsu', 'Datsun', 'Dodge', 'DS Automobiles', 'FAW', 'Ferrari', 'Fiat', 'Fisker', 'Ford',
...
function randomizer(array: string[]): string {
  const index: number = Math.floor(Math.random() * array.length);

  return array[index];
}

export function nameRandomizer(): string {
  return `${randomizer(carBrand)} ${randomizer(carModel)}`;
}

export function colorRandomizer(): string {
